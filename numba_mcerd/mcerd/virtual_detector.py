...
from numba_mcerd.mcerd import ion_simu, enums, misc, rotate, erd_detector
...
def hit_virtual_detector(g: o.Global, ion: o.Ion, target: o.Target, det: o.Detector,
    ...
    v_real_lab = o.Vector()
    v_real_lab.p = misc.coord_transform(
    ...
    p_rec_tar = misc.coord_transform(
    ...
    v_real_tar = o.Vector()
    v_real_tar.theta, v_real_tar.fii = rotate.rotate(
        v_virt_tar.theta, v_virt_tar.fii, v_diff_lab.theta, v_diff_lab.fii)

    v_virt_tar.theta, v_virt_tar.fii = rotate.rotate(
    ...
    if ion.status == enums.IonStatus.NOT_FINISHED:
        ...
        dist = erd_detector.get_distance(p_out_tar, v_real_lab.p)
        ...
        ion.theta, ion.fii = rotate.rotate(det.angle, c.C_PI, v_real_lab.theta, v_real_lab.fii)
...
def get_eloss(E: float, ion: o.Ion, target: o.Target, cos_theta: float) -> float:
    ...
    while cont:
        ...
        sto = ion_simu.inter_sto(layer.sto[ion.scatindex], v, enums.IonMode.STOPPING)
        if E - d * sto < 0.0:
            cont = False
        else:
            v2 = math.sqrt(2.0 * (E - d * sto) / ion.A)
            sto2 = ion_simu.inter_sto(layer.sto[ion.scatindex], v2, enums.IonMode.STOPPING)
            E -= 0.5 * (sto + sto2) * d
